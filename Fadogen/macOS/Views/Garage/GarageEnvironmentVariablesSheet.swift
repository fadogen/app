...
/// Sheet displaying Garage S3 environment variables for Laravel/AWS SDK
/// Users can preview and copy .env configuration
struct GarageEnvironmentVariablesSheet: View {
    @Environment(\.dismiss) private var dismiss
    @State private var showCopiedConfirmation = false

    /// Garage S3 connection environment variables
    private var environmentVariables: String {
        ...
        AWS_REGION=garage
        AWS_ACCESS_KEY_ID=\(garageAccessKeyId)
        AWS_SECRET_ACCESS_KEY=\(garageSecretAccessKey)
